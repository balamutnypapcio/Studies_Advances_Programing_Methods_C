...
{
    // Oczekujemy 3 argumentów: nazwa_programu, plik_konfiguracyjny, plik_polecen
    if (argc != 3) {
        cerr << "!!! Blad: Nieprawidlowa liczba argumentow." << endl;
        cerr << "    Uzycie: " << argv[0] << " <plik_konfiguracyjny.xml> <plik_polecen.txt>" << endl;
        return 1;
    }
    const string configFile = argv[1];
    const string commandsFile = argv[2];
    ...
    cout << "--- Faza 1: Wczytywanie konfiguracji z " << configFile << " ---" << endl;
    ...
    cout << "\n--- Faza 4: Przetwarzanie pliku " << commandsFile << " ---" << endl;
    string processed_content = PreprocessFile(commandsFile);
